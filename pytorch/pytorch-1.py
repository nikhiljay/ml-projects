...
d_in = 3 # input dimensions
hidden = 5 # number of nodes in hidden layer
d_out = 1 # output dimensions
lr = 1e-6 # learning rate
epochs = 500 # number of epochs/iterations

x = Variable(torch.randn(d_in, d_in).type(dtype), requires_grad=False) # random tensor to hold inputs
y = Variable(torch.randn(d_in, d_out).type(dtype), requires_grad=False) # random tensor to hold outputs
...
for i in range(epochs):
  output = x.mm(w1).clamp(min=0).mm(w2) # input tensor multiplied by both weights
